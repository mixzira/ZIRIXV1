		...
		if user_id then
			...
			if not vRP.isBanned(user_id) then
				...
				if vRP.isWhitelisted(user_id) then
					...
					deferrals.done("Solicite sua whitelist em nosso Discord! [ discord.gg/Bp6ZMsS ][ ID: "..user_id.." ]")
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
			else
				deferrals.done("Você foi banido! [ Mais informações em: discord.gg/Bp6ZMsS ] ")
				--print(user_id)
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
