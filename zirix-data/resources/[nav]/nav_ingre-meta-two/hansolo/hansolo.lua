...
Perm = Tunnel.getInterface("nav_ingre-meta-two")
-----------------------------------------------------------------------------------------------------------------------------------------
--[ Menu ]------------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterNUICallback("ButtonClick",function(data,cb)
	if data == "comprar-eter" then
		TriggerServerEvent("departamento-comprar","eter")
    elseif data == "comprar-composito-baixa" then
		TriggerServerEvent("departamento-comprar","composito-baixa")
	...
	while true do
		...
		for k,v in pairs(lojas) do
			...
			if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), lojas.x, lojas.y, lojas.z, true ) < 1.2 then
				DrawText3D(lojas.x, lojas.y, lojas.z, "Pressione [~g~E~w~] para ~g~COMPRAR~w~.")
			end
