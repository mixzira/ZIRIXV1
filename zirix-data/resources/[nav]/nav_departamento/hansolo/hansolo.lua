	...
	while true do
		...
		for k,v in pairs(lojas) do
			...
			if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), lojas.x, lojas.y, lojas.z, true ) < 1.2 then
				DrawText3D(lojas.x, lojas.y, lojas.z, "Pressione [~b~E~w~] para acessar a ~b~LOJA DE CONVENIÊNCIAS~w~.")
			end
			
			if distance <= 5 then
				DrawMarker(23, lojas.x, lojas.y, lojas.z-0.96, 0, 0, 0, 0, 0, 0, 0.7, 0.7, 0.5, 101, 212, 255, 150, 0, 0, 0, 0)
				idle = 5
